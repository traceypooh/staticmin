...
import { v1 as uuidv1 } from 'https://esm.sh/uuid@^3.3.2'
...
class Staticman {
  constructor(parameters) {
    ...
    return (async () => {
      ...
      // the values can have substituted any [NEWLINE] character in your private key files to
      // [SPACE] characters. So now update the values to valid PEM format.
      for (const key of ['rsaPrivateKey', 'githubPrivateKey']) {
      ...
      this.rsa.importKey(this.config.get('rsaPrivateKey'), 'private')
  ...
  processMerge (fields, options) {
    ...
    return this.getSiteConfig().then(_config => {
      return true
    }).catch(err => {
